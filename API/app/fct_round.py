...
def add_match_with_update_player_pts_into_db(new_match):
    sql.add_match_into_db(new_match)
    sql.add_last_match_id_into_actual_round()
    # update rank
    id1, pts_player1, id2, pts_player2 = sql.extract_last_match_to_update_pts()
    sql.update_pts_match_player(id1, pts_player1)
    sql.update_pts_match_player(id2, pts_player2)


def erase_pts_tournament_players(players_listing_id):
    for id_player in players_listing_id:
        sql.erase_pts_match_player(id_player)
...
def generate_first_round():
    ...
    add_match_with_update_player_pts_into_db(new_match)
